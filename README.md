<p align="center">
  ...
  <p align="center">
    <strong>A zero-code text analysis toolkit for social science researchers</strong><br>
...
The default UI language is **English**. Use the **Language** toggle at the top of the page to switch to **Chinese (中文)**. All labels, buttons, and instructions update in real time.
...
- Chinese-aware NER: detects Chinese names with title/honorific patterns (e.g., "Zhang Ms." / "Old Wang")
...
|------|------|-------------|
| `sentiment_example.csv` | QuantiKit | 50 Chinese product/service reviews with 3 sentiment labels (positive / negative / neutral) |
| `policy_example.csv` | QuantiKit | 40 Chinese policy text excerpts with 8 policy-instrument labels |
| `interview_example.txt` | QualiKit | Single-person community healthcare interview transcript (Chinese) |
| `interview_focus_group.txt` | QualiKit | 4-person focus group on elderly digital service experiences (Chinese) |
...
5. Go to **Step 4** &rarr; select LLM backend &rarr; enter labels: `正面, 负面, 中性`
...
5. **Step 3**: define RQs (e.g., "Digital literacy barriers") and sub-themes &rarr; optionally use LLM to suggest sub-themes
...
│       ├── quantikit_template.csv
│       └── qualikit_template.csv
│
...
│   ├── sentiment_example.csv     # Product review sentiment (50 rows)
│   ├── policy_example.csv        # Policy instrument classification (40 rows)
│   ├── interview_example.txt     # Single interview transcript
│   └── interview_focus_group.txt # Focus group transcript (4 speakers)
│
...
- Chae, Y. & Davidson, T. (2025). Large Language Models for text classification: From zero-shot to instruction-tuned. *Sociological Methods & Research*.
...
[English](#what-is-socialscikit) | **中文**
...
│
examples/                         # 示例数据集
│   ├── sentiment_example.csv     # 商品评价情感分类（50 条）
│   ├── policy_example.csv        # 政策工具分类（40 条）
│   ├── interview_example.txt     # 单人访谈记录
│   └── interview_focus_group.txt # 焦点小组讨论（4 人）
│
tests/                            # 测试套件（562 个测试）
